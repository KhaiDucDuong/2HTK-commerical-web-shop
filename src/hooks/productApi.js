...
export async function AllProducts() {
  let url = `http://3.17.159.40:8080/api/products/category`;
  const productList = await fetchData(url);
  return productList;
}

export async function findProducts(name) {
  let url = `http://3.17.159.40:8080/api/products/category?search=` + name;
  const productList = await fetchData(url);
  return productList;
}
export async function findProductsByCategory(ID) {
  let url = `http://3.17.159.40:8080/api/products/category?ID=` + ID;
  const productList = await fetchData(url);
  return productList;
}
...
export async function GetProductByID(ID) {
  let url = `http://3.17.159.40:8080/api/products/detail?product=` + ID;
  try {
